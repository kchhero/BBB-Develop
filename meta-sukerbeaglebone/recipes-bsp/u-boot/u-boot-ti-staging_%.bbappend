...
# UBOOT_CUSTOM_CONFIG가 "1"일 경우에만 my-am335x-config.patch를 SRC_URI에 추가합니다.
# Yocto에서 조건부로 변수를 추가하는 전형적인 문법입니다.
SRC_URI:append = "${@' file://mmc_boot_dev_change_to_1.patch' if d.getVar('UBOOT_MMC_BOOT_BUILD_CONFIG') == '1' else ''}"

# do_install:append() {
#     install -d ${D}${sysconfdir}/boot/extlinux
#     install -m 0644 ${WORKDIR}/extlinux.conf ${D}${sysconfdir}/boot/extlinux/extlinux.conf
# }
